...
contract ExpenseManagerContract {
    ...
    function deposit(uint _amount, string memory reason) public payable {
        require(_amount > 0, "Amount should be greater than 0");
        balances[msg.sender] += _amount;
        transactions.push(
            Transaction(msg.sender, _amount, reason, block.timestamp)
        );
        emit Deposit(msg.sender, _amount, reason, block.timestamp);
    }
